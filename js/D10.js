...
/* ESERCIZIO 3
  ...
  splitMe("I love coding") => ritorna ["I", "Love", "Coding"]
*/
...
let split = splitMe("I love Javascript")
console.log(split)
...
let deleted = deleteOne("Javascript",true)
console.log(deleted)
...
let onlyLetters = (str1) => {
  return str1.replace(/[0-9]/g,'');
}

console.log(onlyLetters("Io ho 1 cane"))
...
console.log(isThisAnEmail(prompt()))*/
...
let howManyDays = (days) => {
  let date = Date.parse(days)
  //console.log(date)
  let nowdate = new Date();
  //console.log(nowdate)
  let time = nowdate.getTime();
  //console.log(time)
  let tot = Math.abs(time - date);
  //console.log(tot)
  let totdays = Math.floor(tot / 1000 / 60 / 60 / 24)
  return totdays
}
...
let deleteProp = (obj1, str) => {
  delete obj1[str];
 return obj1;
}

console.log(deleteProp(obj1,str))
...
let sumAllTheYears = (arr) => {
  return arr.reduce((accumulatore, film) => accumulatore + Number(film.Year),0)
}
...
let searchByTitle = (arr, title) => {
  let arr1 = []
  arr.forEach((movie) => {
    if(movie.Title.toLowerCase().includes(title.toLowerCase())){
      arr1.push(movie.Title);
    }
  });
  return arr1;
}
...
let text = () => {
  let text1 = document.querySelectorAll('td')
